module Spree
  class ProductsController < Spree::StoreController
    before_action :load_product, only: [:show, :draftshow, :vote2, :unvote]
    before_action :load_taxon, only: :index
    before_action :authenticate_spree_user!, only: :draftshow
    ...
    def index
      ...
      @products = @products.where(publish: true).where.not(level: 'croquis')
    ...
    def drafts_index
      ...
      @products = @searcher.retrieve_products
      @products = @products.includes(:possible_promotions) if @products.respond_to?(:includes)
      @products = @products.where(publish: true).where(level: 'croquis')
      @taxonomies = Spree::Taxonomy.includes(root: :children)
    end
    ...
    def show
      ...
      @drafts = Spree::Product.all.where(level: 'croquis').shuffle
    end
    def draftshow
      @products = Spree::Product.all.where(level: ['precommande', 'en vente'], publish: true).where.not(id: @product.id)
      @variants = @product.variants_including_master.
      ...
      @drafts = Spree::Product.all.where(level: 'croquis')
    end
